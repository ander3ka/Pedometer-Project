...
mag[index % N] = sqrt((long)xAxis*(long)xAxis + (long)yAxis*(long)yAxis + (long)zAxis*(long)zAxis);
//magnitude = mag[index % N];

//Take the difference of the magnitude and square it
if (index < 1) {
  diff[index % N] = 0;
}
else {
  diff[index % N] = mag[index % N] - mag[(index-1) % N];
}
sqr[index % N] = diff[index % N] * diff[index % N];
...
void report()
{
 ...
 // Serial.print(","); 
  ...
  //Serial.print(",");
  //Serial.print(mag[index % N]); 
 // Serial.print(",");
  //Serial.print(avg[index % M]);
  //Serial.print(",");
  Serial.println(steps);
  
  
  
  //print back the magnitude for fun so we can see if the calculation was done correctly.
